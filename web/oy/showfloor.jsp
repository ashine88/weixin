...
<html>
	...
	<script type="text/javascript">
		function changeList() {
			 ...
			 if(count=='查看缺寝楼层'){
				 ...
				 document.getElementById("changename").value='查看缺寝楼层';
  ...
  <div style=" height: 50px;margin-top: 20px; margin-left :20px; font-size: 20px">
  	...
  	${buildName} &nbsp;不在寝：${buildnotback }人
  	<input type="button" style="width: 130px" onclick="changeList()" value="查看缺寝楼层" id="changename">			
  </div>
  <hr>
  <!-- 只显示不在寝楼层信息 -->
  <div id="div1" style="display: none;">
   ...
   <a href="roomAction_selectAllroom.action?roomId=${r.roomId }&buildName=${buildName}&floorName=${roomName}&floornotback=${r.personIncount}">
		<ul style="width: 50px%; float: left; border: 1px solid #000 ;margin-left: 25px;">
			<li style="float: left; margin-right: 50px; margin-bottom: 20px; text-align: center; margin-top: 5px;">				
					<p style="font-size: 24px;margin-top: 5px;">${r.roomName }</p>
					 <br>
					不在寝：<span style="color: red; font-size: 24px">${r.personIncount }</span>人 <br>
  ...
  <div  id="div2">
  	<c:forEach items="${rlist }" var="r">
    <a href="roomAction_selectAllroom.action?roomId=${r.roomId }&buildName=${buildName}&floorName=${r.roomName}&floornotback=${r.personIncount}">
