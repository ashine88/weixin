 ...
 <head>
	...
	<body>
		...
		<div style="margin-top: 40px;">
			<form action="alertAction_findAlertByChief.action?pe.pageNum=1" method="post">
				...
				<c:forEach items="${currentList }" var="a">
				 ...
				 <tr>
					<td colspan="5" align="center">
						<a href="alertAction_findAlertByChief.action?pe.pageNum=1&onday=${onday }"><button class="mui-btn mui-btn-primary">首页</button></a>&nbsp;
						<c:if test="${pe.pageNum ==1 }">
							<button class="mui-btn mui-btn-primary">上一页</button>
						</c:if>
						<c:if test="${pe.pageNum !=1 }">
							<a href="alertAction_findAlertByChief.action?pe.pageNum=${pe.pageNum-1 }&onday=${onday }"><button class="mui-btn mui-btn-primary">上一页</button></a>
						</c:if>
						&nbsp;
						...
						<c:if test="${pe.pageNum !=pe.maxPage }">
							<a href="alertAction_findAlertByChief.action?pe.pageNum=${pe.pageNum+1 }&onday=${onday }"><button class="mui-btn mui-btn-primary">下一页</button></a>
						</c:if>
						&nbsp;
						<a href="alertAction_findAlertByChief.action?pe.pageNum=${pe.maxPage }&onday=${onday }"><button class="mui-btn mui-btn-primary">末页</button></a>&nbsp;
						共${pe.totalNum }人
					</td>				
				</tr> 
				</c:if>
